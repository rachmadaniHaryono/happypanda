...
class GalleryMetaWindow(ArrowWindow):
    ...
    def __init__(self, parent):
        ...
        self.g_widget = GalleryLayout(self, parent)
    ...
    def show_gallery(self, index, view):  # NOQA
        ...
        def check_left():
            """check_left."""
            # middle of gallery left side
            middle = (index_top_left.y() + index_btm_left.y()) / 2
            # if the width can be there
            left = (index_top_left.x() - self.width() - margin_offset) > 0
        ...
        def check_right():
            """check_right."""
            # middle of gallery right side
            middle = (index_top_right.y() + index_btm_right.y()) / 2
            # if the width can be there
            right = (index_top_right.x() + self.width() + margin_offset) < desktop_w
            # if the top half of window can be there and for the bottom
            top, btm = self._calculate_sides(
                middle=middle, hw=self.height(), margin_offset=margin_offset, desktop=desktop_h)
...
class ChapterList(QTableWidget):
    ...
    def __init__(self, parent):
        ...
        self.horizontalHeader().\
            setSectionResizeMode(0, self.horizontalHeader().ResizeToContents)
        self.horizontalHeader().\
            setSectionResizeMode(1, self.horizontalHeader().Stretch)
        self.horizontalHeader().\
            setSectionResizeMode(2, self.horizontalHeader().ResizeToContents)
...
class GalleryLayout(QFrame):
    ...
    def __init__(self, parent, appwindow):
        ...
        self.chapter_list = ChapterList(self)
    ...
    def apply_gallery(self, gallery):
        ...
        last_read_txt = \
            '{} ago'.format(PrettyDelta(gallery.last_read).format(use_int=True)) \
            if gallery.last_read else "Never!"
