...
class SettingsDialog(QWidget):
    ...
    def initUI(self):  # NOQA
        ...
        info_lbl = QLabel()
        info_lbl.setText(
            '<a href="https://github.com/Pewpews/happypanda">'
    ...
    def restore_options(self):  # NOQA
        ...
        self.extract_gallery_before_opening.setChecked(
            app_constants.EXTRACT_CHAPTER_BEFORE_OPENING)
    ...
    def _grid_view_tab_on_visual_tabwidget(self, tabwidget):
        ...
        visual = tabwidget
        # grid view
        grid_view_general_page, grid_view_layout = self.new_tab("Grid View", visual, True)
        # grid view / popup
        grid_popup, grid_popup_l = self.groupbox("Popup", QFormLayout, grid_view_general_page)
        ...
        grid_tooltip_layout.addRow(
            QLabel('Control what is displayed in the tooltip when hovering a gallery'))
        ...
        self.gallery_size.setToolTip(
            "Changes size of grid in gridview. Remember to re-generate thumbnails! DEFAULT=3")
        gallery_size_l = QHBoxLayout()
        gallery_size_l.addWidget(gallery_size_lbl)
        gallery_size_l.addWidget(self.gallery_size)
        grid_gallery_main_l.addRow(QLabel(
            "Note: A manual re-generation of thumbnails is required. Advanced -> Gallery"))
    ...
    def accept(self):  # NOQA
        ...
        for x in range(self.g_languages.count()):
            ...
            if l and l not in app_constants.G_LANGUAGES:
        ...
        app_constants.EXTRACT_CHAPTER_BEFORE_OPENING = \
            self.extract_gallery_before_opening.isChecked()
        set(
            app_constants.EXTRACT_CHAPTER_BEFORE_OPENING,
            'Application',
            'extract chapter before opening')
        app_constants.OPEN_GALLERIES_SEQUENTIALLY = self.open_galleries_sequentially.isChecked()
        set(
            app_constants.OPEN_GALLERIES_SEQUENTIALLY,
            'Application',
            'open galleries sequentially')
        ...
        if not self.move_imported_def_path.text() or \
                os.path.exists(self.move_imported_def_path.text()):
            app_constants.IMPORTED_GALLERY_DEF_PATH = self.move_imported_def_path.text()
            set(
                app_constants.IMPORTED_GALLERY_DEF_PATH,
                'Application',
                'imported gallery def path')
        app_constants.OPEN_RANDOM_GALLERY_CHAPTERS = self.open_random_g_chapters.isChecked()
        set(
            app_constants.OPEN_RANDOM_GALLERY_CHAPTERS,
            'Application',
            'open random gallery chapters')
        ...
        exts_list = []
        valid_ext = [
            self.ignore_folder, self.ignore_zip, self.ignore_cbz, self.ignore_rar, self.ignore_cbr]
        ...
        QPixmapCache.setCacheLimit(
            self.cache_size[0] * self.cache_size[1])
    ...
    def init_right_panel(self):  # NOQA
        ...
        self.send_2_trash.setToolTip(
            "When unchecked, files will get deleted permanently and be unrecoverable!")
        app_general_m_l.addRow(self.send_2_trash)
        self.keep_added_gallery = QCheckBox("Remove galleries added in inbox on exit")
        self.keep_added_gallery.setToolTip(
            "When turned off, galleries in inbox will not be deleted on exit")
        ...
        external_viewer_p_info = QLabel(
            ...
            "try changing the arguments sent in the Advanced section")
        ...
        self.external_viewer_path.setPlaceholderText(
            'Right/Left-click to open folder explorer. Leave empty to use default viewer')
        self.external_viewer_path.setToolTip(
            'Right/Left-click to open folder explorer. Leave empty to use default viewer')
        ...
        rar_info = QLabel(
            ...
            'Windows: "unrar.exe" should be in the "bin" directory '
            'if you installed from the self-extracting archive provided on github.\n'
            'OSX: You can install this via HomeBrew. '
            'Path should be something like: "/usr/local/bin/unrar".\n'
            'Linux: Should already be installed. You can just type "unrar". '
            'If it\'s not installed, use your package manager: pacman -S unrar')
        ...
        self.subfolder_as_chapters = QCheckBox(
            "Subdirectiories should be treated as standalone galleries "
            "instead of chapters (applies in archives too)")
        self.subfolder_as_chapters.setToolTip(
            "This option will enable creating standalone galleries "
            "for each subdirectiories found recursively when importing.\n"
            "Default action is treating each subfolder found as chapters of a gallery.")
        extract_gallery_info = QLabel(
            "Note: This option has no effect when turned off "
            "if path to external viewer is not specified.")
        self.extract_gallery_before_opening = QCheckBox(
            "Extract archive before opening (Uncheck only if your viewer supports it)")
        self.open_galleries_sequentially = QCheckBox(
            "Open chapters sequentially (Note: has no effect if path to viewer is not specified)")
        subf_info = QLabel(
            "Behaviour of 'Scan for new galleries on startup' option will be affected.")
        ...
        rename_g_source_l.addRow(QLabel(
            "Check what to include when renaming gallery source. (Same order)"))
        ...
        monitor_info = QLabel(
            'Directory monitoring will monitor the specified directories '
            'for any filesystem events. '
            'For example if you delete a gallery source in one of your monitored directories '
            'the application will inform you and '
            'ask if you want to delete the gallery from the application as well.')
        ...
        def make_login_forms(  # NOQA
                layout,
                exprops,
                baseHen_class,
                partial_txt='You have partial access!'
        ):
        ...
        ehentai_user, ehentai_pass, ehentai_status = make_login_forms(
            ...
            pewnet.EHen,
            (
                "You have partial access (g.e-hentai). "
                "Enter details in the Advanced section to access exhentai."
            )
        )
        ...
        web_downloader_l.addRow(
            QLabel(
                "Leave empty to use default torrent client.\n"
        ...
        self.continue_a_metadata_fetcher = QCheckBox(
            'Skip galleries that has already been processed in auto metadata fetcher')
        ...
        replace_metadata_info = QLabel(
            'By default metadata is appended to a gallery.\n'
            'Enabling this option makes it so that a gallery\'s old data is deleted and '
        ...
        use_gallery_link_info = QLabel(
            "Enable this option to fetch metadata using the currently applied URL on the gallery")
        self.use_gallery_link = QCheckBox('Use currently applied gallery URL')
        self.use_gallery_link.setToolTip(
            "Metadata will be fetched from the current gallery URL "
        ...
        fallback_source_info = QLabel(
            "Specify which sources metadata fetcher should fallback to "
            "when a gallery is not found.")
        ...
        # Visual / Grid View
        self._grid_view_tab_on_visual_tabwidget(tabwidget=visual)
        # Visual / Style
        style_page = QWidget(self)
        visual.addTab(style_page, 'Style')
        # Visual tab widget settings
        ...
        scroll_speed_spin_box.setToolTip(
        ...
        cache_size_spin_box.setToolTip(
            'This can greatly reduce lags/freezes in the grid view. '
        ...
        exh_info = QLabel(
            "Warning: Only enter these values "
            "if you weren't able to gain access to exhentai by logging in at Web->Logins!")
        ...
        def rebuild_thumbs():
            ...
            if confirm_msg.exec_() == QMessageBox.Yes:
                clear_cache_confirm = QMessageBox(
                    QMessageBox.Question,
                    '',
        ...
        rebuild_thumbs_info = QLabel(
            "Clears thumbnail cache and rebuilds it, which can take a while. "
            "Tip: Useful when changing thumbnail size.")
        ...
        g_data_regex_fix_lbl = QLabel(
            "Rename a gallery through regular expression. "
        ...
        def init_import():
            path = QFileDialog.getOpenFileName(
                self, 'Choose happypanda database file', filter='*.hpdb')
            path = path[0]
            if len(path) != 0:
                app_popup = AppDialog(self.parent_widget)
