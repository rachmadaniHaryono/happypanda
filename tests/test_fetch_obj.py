...
def test_check_single_gallery(cond, is_idx_end):
    ...
    #
    from happypanda.fetch_obj import FetchObject
...
def test_get_gallery_list_and_status(raise_type_error):
    ...
    path.name = name
    #
    with mock.patch('happypanda.fetch_obj.scandir') as m_sd:
        if raise_type_error:
...
def test_create_gallery_from_path_as_chapter(path_is_dir, scandir_return_list, path_ext):
    ...
    with mock.patch('happypanda.fetch_obj.os') as m_os, \
            mock.patch('happypanda.fetch_obj.scandir') as m_scandir, \
            mock.patch('happypanda.fetch_obj.app_constants') as m_app_constants:
...
def test_create_gallery_from_path_with_subfolder_as_gallery(path_type):
    ...
    with mock.patch('happypanda.fetch_obj.os') as m_os, \
            mock.patch('happypanda.fetch_obj.app_constants') as m_app_constants, \
            mock.patch('happypanda.fetch_obj.utils') as m_utils:
        m_app_constants.ARCHIVE_FILES = ('.zip', '.cbz')
